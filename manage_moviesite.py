...
def ffmpeg_set_title(fname, title):
    ...
    command_array = ['ffmpeg', '-i', fname, '-metadata', 'title={}'.format(title), '-c:a', 'copy', '-c:v', 'copy', tmp_fname]
    try:
        subprocess.run(command_array, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, encoding='utf8', check=True)
...
def mkv_set_title(fname, title):
    ...
    '''
    if title:
        command_array = ['mkvpropedit', '{}'.format(fname), '--edit', 'info', '--set', 'title={}'.format(title)]
    else:
...
class VideoParser():
    ...
    def login(self):
        ...
        #
        # get login page
        response = self.session.get(self.args.address + URL_LOGIN, auth=('username', 'password'))
        ...
        payload = {'username': self.args.user, 'password': self.args.password, 'csrfmiddlewaretoken':csrftoken}
        # prepare header
        self.header = {
        ...
        response = self.session.post(self.args.address + URL_LOGIN, data=payload, headers=self.header)
    ...
    def parse_directory(self, parse_file, thepath):
        ''' Parse directory '''
        print('Parse directory "%s"%s' % (thepath, ' and subdirectories' if not self.args.no_recurs else ''))
    ...
    def parse_file(self, fname):
        ...
        if ffprobe_result.return_code == 0:
            ...
            print('   container: {0} | {1} , rate: {2}, res: {3}, duration: {5}, filesize: {4}'.format(\
                    probe['format']['format_name'], probe['smart_streams'], \
            ...
            if probe['title']:
                ...
                if base != probe['title']:
                    ...
                    if self.args.fix_title:
                        ...
                        if set_video_title(fname, probe['format']['format_name'], title):
...
def main():
    ...
    parser.add_argument('-a', '--address', default='http://diskstation:6062', \
    ...
    parser.add_argument('--no-recurs', action='store_true', \
        help='don\'t parse sub-directories')
...
#    parser.add_argument('--fix-title', action='store_true', \
    parser.add_argument('--fix-title', default='', \
