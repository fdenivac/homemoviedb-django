...
class Command(BaseCommand):
    ...
    def add_or_update_team(self, movie):
        ...
        for cast in creds.cast:
            try:
                team = Team(
                    movie=movie, job="Actor", name=cast.name, extension=cast.character
                )
                if not self.options["simu"]:
                    team.save()
    ...
    def add_or_update_poster(self, movie, original_language):
        ...
        if len(images["posters"]) > settings.MAX_POSTERS:
            print(
                "  Ignore {0} on {1}".format(
                    len(images["posters"]) - settings.MAX_POSTERS,
                    len(images["posters"]),
                )
            )
        for num, poster in enumerate(images["posters"][: settings.MAX_POSTERS]):
            try:
                ...
                url = "{0}{1}{2}".format(
                    self.config["images"]["base_url"], "w500", rel_path
                )
    ...
    def search_tmdb(self, moviename, year=None):
        ...
        for res in results:
            original_title = (
                res.original_title
                if res.title.lower() != res.original_title.lower()
                else ""
            )
    ...
    def get_tmdb(self, idmovie):
        ...
        original_title = (
            ...
            if results.title.lower() != results.original_title.lower()
            else ""
        )
    ...
    def parse_file(self, fname):
        ...
        movie = self.add_or_update_movie(
            fname,
            "OK",
            fmt["size"],
    ...
    def parse_directory(self, thepath):
        ...
        print(
            'Parse directory "%s"%s'
            % (thepath, " and subdirectories" if not self.options["no_recurs"] else "")
        )
