...
def duplicated(request):
    ...
    titles = Movie.objects.filter(file_status='OK') \
    ...
    # and select movies
    movies = Movie.objects.filter(file_status='OK').annotate(dtitle=Lower('title')).filter(dtitle__in=titles).order_by('title')
    paginator, movies, page = paginate(request, movies)
    context = {
...
def without_poster(request):
    ...
    movies = Movie.objects.filter(file_status='OK').annotate(images=Count('poster')).filter(images=0).order_by('title')
    paginator, movies, page = paginate(request, movies)
    context = {
...
def no_viewed(request, volume, order):
    ...
    movies = Movie.objects.filter(file_status='OK', file__istartswith=volume, viewed=0).order_by(*order)
    paginator, movies, page = paginate(request, movies)
...
def no_viewed_genres(request, volume, order):
    ...
    genres = {}
    qsgenres = Movie.objects.filter(file_status='OK', file__istartswith=volume, viewed=0).values('genres').distinct()
...
def ajax_no_viewed_genre(request):
    ...
    movies = Movie.objects.filter(file_status='OK', file__istartswith=volume, viewed=0, genres__contains=genre).order_by(*order)
    print(len(movies))
...
def movies_viewed(request, volume, order):
    ...
    order = set_order(order)
    movies = Movie.objects.filter(file_status='OK', file__istartswith=volume, viewed__gt=0).order_by(*order)
    paginator, movies, page = paginate(request, movies)
    vol_label = get_volume_alias(volume)
    onwhere = 'on "{}"'.format(vol_label) if vol_label else ''
    context = {
...
def movies_genres(request, order):
    ...
    context = {
        'genres':num_genres,
        'volumes':settings.VOLUMES, 'volume':'', 'query':'', 'order':order,
    }
...
def movies_genre(request, genre, order):
    ''' movies for a genre '''
    order = set_order(order)
    movies = Movie.objects.filter(genres__contains=genre, file_status='OK').order_by(*order)
...
def searchbypath(request, volume, query, order):
    ...
    if volume in [None, '', settings.ALL_VOLUMES]:
        if query:
            movies = Movie.objects.filter((Q(title__contains=query) | Q(original_title__contains=query)) & Q(file_status='OK')).order_by(*order)
        else:
            movies = Movie.objects.filter(file_status='OK').order_by(*order)
    else:
        if query:
            movies = Movie.objects.filter(Q(file__istartswith=volume, ) & (Q(title__contains=query) | Q(original_title__contains=query))).order_by(*order)
        else:
...
def persons_most_credited(request, jobcriter):
    ...
    persons = Team.objects.filter(job=jobcriter).values('name').annotate(total=Count('id')).order_by('-total')
    context = {
        'table_type':'Top {} of {} (total of {})'.format(len(persons[:settings.NUM_TOP]), jobcriter, len(persons)),
        'jobcriter': jobcriter,
...
def movies_jobperson(request, job, person):
    ...
    if job:
        movies = Movie.objects.filter(team__job=job, team__name__contains=person, file_status='OK').distinct().order_by('title')
    else:
        movies = Movie.objects.filter(team__name__contains=person, file_status='OK').distinct().order_by('title')
    paginator, movies, page = paginate(request, movies)
    forjob = 'in job "{}"'.format(job) if job else ''
    context = {
